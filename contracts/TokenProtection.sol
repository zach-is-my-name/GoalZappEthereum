...
contract TokenProtection is EscrowRole, AionRole{
  using SafeMath for uint256;
   ...
   event CheckRequire(uint _amount, uint256 _protectedTokens, bool lessThanProtected);
    ...
    function timeProtectTokens(address _address, uint256 _amount) public onlyEscrowRole 
      returns(bool) {
      protectedTokens[_address] = protectedTokens[_address].add(_amount);
     // return true;
    }

    function removeTokenProtection(address _address, uint256 _amount) onlyAionRole public
      returns (bool) {
    ...
    function setEscrowAddress(address _escrowAddress) public returns (bool) {
