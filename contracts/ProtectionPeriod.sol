  ...
  using SafeMath for uint256;
   ...
   event CheckRequire(uint _amount, uint256 _protectedTokens, bool lessThanProtected);
    ...
    modifier checkProtectedTokens (uint256 amount) {
      ...
      if (amountProtected(msg.sender) > 0) {
       // emit TransferChecked("Transfer Checked!!! || Protected > 0");
        require (amount < amountProtected(msg.sender), "your tokens are under protection period, check timeToLiftProtection() for time until you have tokens available, and/or check amountProtected to see how many of your tokens are currently under the protection period" );
	//emit CheckRequire(amount, protectedTokens[msg.sender], amount < protectedTokens[msg.sender]);
      }
      _;
    }

    function timeProtectTokens(address _address, uint256 _amount) public
      returns(bool) {
      protectedTokens[_address] = protectedTokens[_address].add(_amount);
     // return true;
    }

    function removeTokenProtection(address _address, uint256 _amount) public
      returns (bool) {
